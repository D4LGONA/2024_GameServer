...
enum EVENT_TYPE { EV_RANDOM_MOVE, EV_SEND_BYE};
...
void worker_thread(HANDLE h_iocp)
{
	while (true) {
		...
		case OP_NPC_MOVE: {
			...
			if (true == keep_alive and false == clients[key].isdoingAI) {
		...
		case OP_AI_HELLO: { // 이벤트 큐에서 hello를 실행했을 때
			...
			TIMER_EVENT ev{ key, chrono::system_clock::now(), EV_RANDOM_MOVE, 0 };
			timer_queue.push(ev);
			TIMER_EVENT ev2{ key, chrono::system_clock::now() + 1s, EV_RANDOM_MOVE, 0 };
			timer_queue.push(ev2);
			TIMER_EVENT ev3{ key, chrono::system_clock::now() + 2s, EV_RANDOM_MOVE, 0 };
			timer_queue.push(ev3);
			TIMER_EVENT ev4{ key, chrono::system_clock::now() + 2s, EV_SEND_BYE, 0 };
